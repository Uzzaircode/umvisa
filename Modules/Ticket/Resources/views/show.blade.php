...
@section('content') {{-- // Status Codes 1 = Draft - yellow 2 = Submitted to HOD - green 3 = Approved
by HOD - blue 4 = Rejected by HOD - red 5 = Submitted to Dasar - green 6 = Approved by Dasar - blue 7 = Rejected by Dasar
- red 8 = Submitted to PTM - green 9 = Approved by PTM - blue 10 = Rejected by PTM -red // Buttons 1. approve_hod = Approve
by HOD 2. reject_hod = Reject by HOD 3. approve_dasar = Approve by Dasar 4. reject_dasar = Reject by Dasar 5. approve_ptm
= Approve by PTM 6. reject_ptm = Reject by PTM --}}
    ...
    <div class="col-lg-6 col-md-12">
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">
            <i class="fe fe-tag"></i> {{$ticket->subject}}
          </h3>
  ...
  @include('ticket::components.approve-reject-buttons')
        ...
        <div class="card-body">
          ...
          <div class="form-group">            
            <select name="it_person_id" id="" class="form-control selectize">
              <option value="0">Select to which staff you want to assign this ticket</option>
                 @foreach($it_persons as $it)
          ...
          <div class="form-group text-right">
            ...
            <button type="submit" name="assign_ticket" class="btn btn-sm btn-success">
                    <i class="fe fe-user-check"></i> Assign Ticket</button> 
          </div>
          @endif
              ...
              <td colspan="2">
                <div class="row gutters-sm" id="attachment">
                  ...
                  <div class="col-6 col-sm-4">
                    @if (pathinfo($t->path, PATHINFO_EXTENSION) == 'jpg' || pathinfo($t->path, PATHINFO_EXTENSION) == 'png' || pathinfo($t->path, PATHINFO_EXTENSION) == 'jpeg')
                    <a href="{{asset($t->path)}}" data-toggle="lightbox" data-gallery="attachment-gallery">
                        <img src="{{asset($t->path)}}" width="200px" class="img-fluid">
                    </a> 
                    @endif 
                    @if(pathinfo($t->path, PATHINFO_EXTENSION) == 'doc' || pathinfo($t->path,
                    PATHINFO_EXTENSION) == 'docx')
                    <a href="{{asset($t->path)}}">
                      <img src="{{asset('img/file-icons/doc.png')}}" alt="">
